...
modded class SCR_NearbyContextDisplay
{
	override void DisplayUpdate(IEntity owner, float timeSlice)
	{
		...
		m_pInteractionHandlerComponent.GetNearbyShowableContextList(contexts);
		array<UserActionContext> contexts2 = {};
		m_pInteractionHandlerComponent.GetNearbyAvailableContextList(contexts2);
		if (contexts2.Count() > 0)
		{
			foreach (UserActionContext cont : contexts2)
			{
				contexts.Insert(cont);
			}
		}
		/*array<UserActionContext> contexts1 = {};
		m_pInteractionHandlerComponent.GetNearbyUnavailableContextList(contexts1);
		if (contexts1.Count() > 0)
		{
			foreach (UserActionContext cont : contexts1)
			{
				contexts.Insert(cont);
			}
		}*/
